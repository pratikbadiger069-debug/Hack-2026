...
export function AIProviderModal({ isOpen, onClose }: AIProviderModalProps) {
  ...
  const [customPrompt, setCustomPrompt] = useState('Verify live connection and echo back confirmation.');
  ...
  const runConnectionTest = async () => {
    ...
    if (!cleaned || cleaned.length < 8) {
      setTestResult({
        ...
        provider: selectedProvider === 'gemini' ? 'Google Gemini AI' : selectedProvider === 'openai' ? 'OpenAI' : 'Anthropic Claude',
    ...
    try {
      const res = await fetch('/api/ai/test', {
        ...
        body: JSON.stringify({
          ...
          apiKey: cleaned,
          testPrompt: showPromptInput && customPrompt.trim() ? customPrompt.trim() : 'Connection probe test. Respond: "Connection Verified".',
        }),
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    const cleaned = cleanApiKey(keyInput);
    setAIKey(selectedProvider, cleaned);
    setActiveAIProvider(selectedProvider);

    // Also persist to server database if authenticated
    try {
      ...
      // client-side state is already updated
    }
      ...
      <div className="w-full max-w-xl bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-start gap-3 p-3 bg-blue-50/60 border border-blue-100 rounded-lg text-xs text-blue-900">
            ...
            <div>
              <span className="font-semibold">Zero-Leakage BYOK Architecture:</span> Keys are sanitized, validated with provider servers, and securely stored. SkillBridge AI never logs your private tokens.
            </div>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={keyInput}
                  onChange={(e) => {
                    setKeyInput(cleanApiKey(e.target.value));
                    setTestResult(null);
                  }}
                ...
                <button
                  ...
                  className="absolute right-1 top-1 bottom-1 px-3 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-[11px] font-semibold flex items-center gap-1 transition-colors disabled:opacity-50"
                >
                  {testing ? (
            ...
            <div>
              ...
              >
                <span>{showPromptInput ? 'Hide custom test prompt' : '+ Customize diagnostic test prompt'}</span>
              </button>
              {showPromptInput && (
                <div className="mt-2">
                  <input
            ...
            {testResult && (
              <div className={`p-3.5 rounded-lg border text-xs space-y-2 animate-in fade-in duration-150 ${getStatusBadgeStyle(testResult.status)}`}>
                ...
                {testResult.echoResponse && (
                  <div className="mt-2 p-2 bg-white/90 rounded border border-emerald-200 text-[10px] font-mono flex items-center gap-1.5 text-slate-800">
                    <Terminal className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                    <span className="font-semibold text-emerald-700">Ping Echo:</span>
                    <span className="truncate">&quot;{testResult.echoResponse}&quot;</span>
                  </div>
                )}
              </div>
            )}
