...
function VerifyEmailContent() {
  ...
  useEffect(() => {
    ...
    if (qToken) {
      ...
      if (qEmail) {
        verify(qEmail, qToken);
      }
    }
  }, [searchParams]);

  const verify = async (targetEmail: string, targetToken: string) => {
    setError(null);
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-xs text-slate-600">
          Confirm your account email address to activate your full SkillBridge access.
        </p>
      </div>
          ...
          {success ? (
            <div className="text-center space-y-4">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                Your email <strong className="text-slate-800">{email}</strong> has been authenticated. You can now access your dashboard and builder workspace.
              </p>
